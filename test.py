...
for deg in degres:
    for _ in range(number_of_functions):
        for num,layer,anzats_reps,n_qubit in zip(nums,layers_list,anzats_reps_list,n_qubits):
            target_function = functions.function_to_learn(degree=deg)
            x = torch.linspace(-torch.pi, torch.pi, steps=800, device=device)
